...
import { calculateSkillMatch } from '../utils/mathUtils.js';

export const createProject = async (req, res, next) => {
  try {
    const { title, description, requiredSkills, roleType, seniority, workMode, duration, openings, compensation, applicationDeadline, githubUrl } = req.body;
    const project = await Project.create({
      title, description, requiredSkills: requiredSkills || [], ownerId: req.user.id,
      roleType: roleType || '', seniority: seniority || '', workMode: workMode || '',
      duration: duration || '', openings: openings || 1, compensation: compensation || '',
      githubUrl: githubUrl || '', applicationDeadline: applicationDeadline || undefined
    });
...
export const getProjects = async (req, res, next) => {
  try {
    const projects = await Project.find({})
      .populate({ path: 'ownerId', select: 'name email username avatar isDemo', match: { isDemo: { $ne: true }, email: { $not: /demo/i } } })
...
export const applyToProject = async (req, res, next) => {
  try {
    ...
    if (matchScore >= 50) {
      const application = await Application.create({ projectId, userId: req.user.id, matchScore });
...
export const getUserApplications = async (req, res, next) => {
  try {
    const applications = await Application.find({ userId: req.user.id }).populate('projectId', 'title createdAt');
    const formatted = applications.map(app => ({
      id: app.projectId?._id, applicationId: app._id, title: app.projectId?.title || 'Unknown Project',
      status: app.status, appliedDate: app.appliedAt.toISOString().split('T')[0]
    }));
    res.json(formatted);
  } catch (error) { next(error); }
};
...
export const deleteProject = async (req, res, next) => {
  try {
    const project = await Project.findById(req.params.id);
    if (!project) return res.status(404).json({ message: 'Project not found' });
    if (project.ownerId.toString() !== req.user.id) return res.status(403).json({ message: 'User not authorized to delete this project' });
