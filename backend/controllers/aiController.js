...
exports.runMatchmaker = async (req, res) => {
  try {
    ...
    // Agentic Step 1: Pre-filter using MongoDB $in on skills (much faster)
    let filteredUsers = await User.find({
      skills: { $in: keywords.map(k => new RegExp(k, 'i')) }
    })
      .select('name skills rating projectsCompleted challengesSolved githubUrl karma')
      .limit(20)
      .lean();

    // Fallback if skill filter returns too few
    ...
    const minimalUsers = filteredUsers.map(u => ({
      _id: u._id,
      name: u.name,
      skills: u.skills,
      rating: u.rating
    ...
    // Agentic Step 2: Reasoning & Extraction with timeout
    ...
    try {
      const agentResponse = await Promise.race([
        agenticMatchmaker(requirements, talentPoolStr),
        timeoutPromise
      ]);
      res.json(agentResponse);
    } catch (aiErr) {
      if (aiErr.message === 'AI_TIMEOUT') {
        // Return pre-filtered results directly without AI ranking
        ...
        res.json({
          logs: ['AI ranking timed out. Showing direct skill matches.'],
